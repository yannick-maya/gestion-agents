...
<div class="container my-4">
    ...
    <div class="row">
        <div class="col">
            <h2 class="display-4 text-center text-danger">Candidatures Rejetées</h2>
        </div>
    </div>
            ...
            <tbody>
                @forelse ($dossiers as $dossier)
                    <tr>
                        ...
                        <td>
                            <span class="badge 
                                {{ $dossier->statut_dossier == 'rejeté' ? 'bg-danger' : ($dossier->statut_dossier == 'valide' ? 'bg-success' : 'bg-warning') }}">
                                {{ ucfirst($dossier->statut_dossier) }}
                            </span>
                        </td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="9" class="text-center text-muted">
                            <i>Aucune candidature rejetée trouvée.</i>
                        </td>
                    </tr>
                @endforelse
            </tbody>
